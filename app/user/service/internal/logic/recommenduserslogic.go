...
func (l *RecommendUsersLogic) RecommendUsers(in *user.RecommendUsersRequest) (resp *user.RecommendUsersResponse, err error) {
	...
	if len(in.UserSelfTagId) == 0 {
		//没有选择用户自定义标签则根据系统标签查询
		resp, err = QueryUsersByConditions(l.svcCtx.DB, in.PageNo, in.PageSize, in.SystemTagId, in.UserId)
		if err != nil {
			return nil, err
		}

	} else {
		//选择了用户自定义标签 , 则根据用户自定义标签查询
		resp, err = QueryUsersByConditions(l.svcCtx.DB, in.PageNo, in.PageSize, in.UserSelfTagId, in.UserId)
		if err != nil {
			return nil, err
		}
	}
...
func QueryUsersByConditions(db *gorm.DB, pageNo, pageSize int64, tagIds []uint64, userID uint64) (*user.RecommendUsersResponse, error) {
	...
	if err := db.
		...
		Joins("JOIN tb_user_tag ON users.id = tb_user_tag.user_id AND users.deleted_at IS NULL AND tb_user_tag.deleted_at IS NULL").
		Joins("JOIN tb_tag ON tb_user_tag.tag_id = tb_tag.id").
		Where("tb_tag.id IN (?) AND tb_tag.type != 'OFFICIAL' AND users.id != ? AND users.deleted_at IS NULL", tagIds, userID).
